...
%
%   Notes
%     - The exe file-version and the installer wizard version are both stamped
%       from xrdc.version() (so they read e.g. 1.1.0, not the mcc/installer
%       default 1.0.0.0 / 1.0). Bump xrdc.version() and rebuild to change them.
...
%       (single write, no re-open) and distribute exe + XRDC.ctf together.
    ...
    % --- Version stamped into the exe + installer, from the single source of
    %     truth (xrdc.version). Put the package on the path so we can call it. ---
    if isfolder(fullfile(repoRoot, "+xrdc"))
        addpath(repoRoot);
    end
    appVersion = xrdc.version();              % e.g. "1.1.0"
    exeVersion = char(appVersion + ".0");     % ExecutableVersion wants 4 parts: "1.1.0.0"
